...
func TestGolangCIExample(t *testing.T) {
	a := goheader.New(
		goheader.WithTemplate(`{{ AUTHOR }} {{ MY COMPANY }} {{ YEAR }}
SPDX-License-Identifier: Apache-2.0
...
limitations under the License.`),
		goheader.WithValues(map[string]goheader.Value{
			...
			"AUTHOR": &goheader.RegexpValue{
				RawValue: `.*@mycompany\.com`,
			},
		}))
	issue := a.Analyze(header(`author@mycompany.com mycompany.com 2020
SPDX-License-Identifier: Apache-2.0
